...
**進捗**: フェーズ 0〜5 は実装済み。フェーズ 6 は段 0（スコープ）・メモ化・D3（`if` のインライン評価）と
スタック継ぎ足しまで実装済み（段 2 相当を含む）。残りは段 1 本来の形（フレーム引数の引き回し）と段 3（ループ）。
...
`for` / `while` の const 化。**段 2 の後でなければ入れられない**
（止まらない const ループを止める手段が要る）。
...
**まだ残っていること**: 1 呼び出しにつきモジュールコンテキストの clone が 1 回残っている
（呼び出し元の鎖ではなくなったので深さに比例はしないが、モジュール規模には比例する）。
これを 0 にするには、フレームがスコープを**借りる**（`&Context` を持つ）必要があり、
`eval_const_*` の約 20 関数にフレーム引数を通す本来の段 1 の作業になる。
D1(a) のカーソル（`(module, スコープ名)` を都度解決）はそのときに効く。

#### 未解決

- 本体の中で定義された const 関数（`Outer(N) = ... Inner(M) = M + N ...`）は
  今 `NameError` になる。クロージャとして扱うのか、禁止するのかを決める必要がある。
  今は `def_scope` を持たせずメモ化から外してあるだけで、意味論は決まっていない
- ユーザークラスの const メソッドは、引数側のバグを直して**パラメータだけを使うものは畳めるように
  なった**。残るのは**クラス自身の const を読む**もの（`C.Base + N`）で、フレームの親が
  モジュールスコープなのが原因。クラススコープを親にすると今度はモジュールの名前が見えなくなるので、
  段 1（フレームに明示的な自由名の解決先を持たせる）を待つ
...
| 7 | フェーズ 6（評価器再設計） | 段 0・D3・メモ化・スタック継ぎ足し（段 2 相当）まで実装済み。残り段 1・段 3 |

フェーズ 6 の段取りは「段 0: スコープを直す → 段 1: フレームのヒープ化 → 段 2: ステップ予算 →
段 3: ループ」。段 0 は正しさのバグ（別モジュールの const 関数が `NameError`）を単体で直せる。
