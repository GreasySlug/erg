...
そこで**再帰まで済ませたあとの最長行が、元の3/4以下になっていなければ割るのをやめる**。
分割は最低でも2行を消費するので、数桁のために払う代償ではない。

閾値はリポジトリ全 `.er` 576ファイルでの実測から決めた。
測る対象は**整形前**のツリー（`062b44b2^`）である
— フォーマッタが初めて触るときに見るのがその形であり、
整形後に測り直すと `wrap` が早期 return して分割判断が3件しか起きない:
...
| 50〜59% | 1 |
| 60〜69% | 3 |
| **70〜79%** | **1**（上の semver、79%） |
| **80〜89%** | **2**（81%、`\` 継続を含む） |

**68% と 79% の間が空いている。** 残すべき84件は全て68%以下、
読みにくくなる3件が 79 / 81 / 81%。3/4（75%）はその谷のちょうど中に落ちる。
