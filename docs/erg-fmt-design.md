...
| R4 | 複数行文字列リテラルの `lineno` が壊れている | **【確認済み・既存バグ】** §10.1 を参照。`\n` エスケープを含む `"""..."""` は `lineno = 0` になり、`Locational::loc()` が `Location::Unknown` を返す。Phase 2 の行構造復元（§2.4）が破綻するので、着手前に直す必要がある |
...
**修正**: 行数は復号後の `cont` ではなく**原文**から数えればよい。
Step 2 で `raw_since_token_start()` を入れたので、必要なデータはすでに手元にある。
上の3ケースすべてで正しい値になることを確認済み。
