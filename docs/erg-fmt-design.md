...
> 現状 CRLF が保存されているのは `render` が恒等関数だからで、
> 実装が入った時点でこの処理を同時に入れないと壊れる。
...
| R10 | 整形すると CRLF が LF に化ける | `raw_text()` が正規化後のバッファ由来のため。§5.4.1 の復元処理を `render` と同時に入れる。**入れ忘れると Windows で全行が差分になる** |
