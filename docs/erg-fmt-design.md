...
### 2.8 `#[ ]#` コメントの直後に空白を置けない 【レキサの制約】

`#[]#print! 0` は lex できるが、`#[]# print! 0` は
`SyntaxError: invalid character: ' '` になる。
コメントを読み終えた時点でレキサはまだインデント走査中で、
そこに現れた空白を弾いてしまう。

当初これを**行頭のブロックコメント限定**の制約だと考えていたが、**行のどこでも同じ**だった:

| ソース | コンパイラの lex |
| ------ | ---------------- |
| `print! #[]#0` | 通る |
| `print! #[]# 0` | `invalid character: ' '` |
| `a = 1 #[]#+ 2` | 通る |
| `a = 1 #[]# + 2` | `invalid character: ' '` |

**しかもこれは §6 の検証をすり抜ける。** フォーマッタは全段階で
`keep_comments` を付けて lex しており、コメントがトークンとして残る読み方では
`#[]# 0` は何の問題もなく通るからである。つまり**検証はコンパイラと違う設定の
レキサで回っていた**。`tests/should_ok/comment.er` を整形した結果、
検証を素通りして lex できないファイルが書き出されて発覚した。

対策は2段構え:

1. 空白規則で**コメントの直後は常に詰める**（`p == Comment => Space::None`。
   fixity 規則より上に置く。ここの空白は「別のプログラム」ではなく「プログラムではない」ため）
2. 検証に**コメントを捨てた読み方での比較**を足す（§6）。1 はこの1件を塞ぐだけだが、
   2 はこの種の穴をまとめて塞ぐ
...
`:=` と `**` が**逆向きに傾いている**のが要点で、片方に合わせた規則は
もう片方で必ず外れる。

（この表は当初 `examples` と `tests` の257ファイルだけで測っており、
`:=` は 74 対 42 と拮抗して見えていた。空ける流儀が圧倒的なのは
`.d.er` の宣言（stdlib 281ファイルだけで 1851 対 59）で、
そこが標本から丸ごと抜けていた。結論は変わらない
— どちらの流儀も大量にある — が、傾きの向きは逆だった）
...
#### 5.7.5 割り甲斐の判定 【実装時に追加】

上の 1〜5 だけだと、**割っても長さが減らない行を割ってしまう**。
`erg fmt` が [semver.er](../crates/erg_compiler/lib/std/semver.er) に対して出した例:
...
```erg
            [                        # 121桁が5行になり
                ...
                patch,
                pre
            ] -> major.isnumeric() and minor.isnumeric() and patch.isnumeric() and pre.isalnum()
                                     # 一番長い行はまだ96桁
```

長さの出どころは `and` の連鎖で、カンマでは割れない。パターンのほうを展開しても
**問題が移動するだけで解決していない**。しかも1行上の3要素版（98桁）は割られずに
残るので、並んでいたものが揃わなくなる。

判定は2段構えである。分割後の部分を「括弧の**内側**（要素の行）」と
「**外側**（括弧の前後、分割が触らないテキスト）」に分け、再帰まで済ませてから
それぞれの最長行を測る。

1. **最長行が内側にある** → そのまま残す。
   括弧を割ったことが行を短くしたのだから、割り甲斐の議論はそこで終わっている
2. **最長行が外側にある** → 分割は長さの出どころに当たっていない。
   このときだけ**元の3/4以下**を要求する

上の semver は 2 に落ちる。パターンを展開しても最長行は矢印の後ろの `and` 連鎖のままで、
121桁が96桁になるだけ（79%）なので却下される。

閾値はリポジトリ全 `.er` 576ファイルでの実測から決めた。
測る対象は**整形前**のツリー（`062b44b2^`）である
— フォーマッタが初めて触るときに見るのがその形であり、
整形後に測り直すと `wrap` が早期 return して分割判断がほとんど起きない。
分割判断は87件で、内訳は:
...
外側だった4件の比率は 3x% / 4x% / **79%** / **81%** で、
**49% と 79% の間が大きく空いている**。3/4（75%）はその谷に置いた。

判定は**再帰の後**に行う。要素がさらに割れる場合、最終的な形が割り甲斐の対象だからである。

#### 5.7.6 パラメータリストは詰める 【実装時に追加】

閉じ括弧の直後が `->` / `=>` なら、その括弧は**呼び出しの引数**ではなく
**関数型のパラメータリスト**である。トークン列では閉じ括弧まで両者を区別できないので、
矢印の有無がそのまま判定になる。

パラメータリストは1行1要素に積まずに、幅いっぱいまで**詰める**:

```erg
    __call__: (
        days := Nat, seconds := Nat, microseconds := Nat, milliseconds := Nat, minutes := Nat,
        hours := Nat, weeks := Nat
    ) -> .TimeDelta
```

引数は上から順に読むものだが、パラメータリストは**集合として見る**
— 中の1つを探し、本当に読んでいるのはそれが属するシグネチャのほうである。
1要素1行にすると宣言が段落になる。本体を持つ定義ならそれでよいが、
`.d.er` は宣言が縦に並んだものなので、1メンバ9行では一覧として読めなくなる。

実測では、リポジトリ全体でフォーマッタが作る分割87件のうち**84件がこの形**で、
そのほぼ全部が `.d.er` にある。積むか詰めるかは事実上この1構文の見た目を決める選択だった。
stdlib の行数は整形前比で **+414行 → +184行**、100桁超えの行は 38本 → 37本になる。

幅に収まりきらない要素は単独の行に置かれ、[`wrap`] がもう一度割る機会を得る。
...
#### 6.0.1 レキサの設定を2つとも通す 【実装時に追加】

上の疑似コードは `keep_comments` の読み方だけで検証している。これでは**足りない**。
§2.8 のとおり、コメントをトークンとして残す読み方と捨てる読み方とでは
**受理する入力が違う**。前者しか見ていないと、後者が拒否する出力を
いくらでも吐けてしまう。守るべきなのは `erg fmt` ではなく `erg` である。

```rust
compare(&before, &after)?;                      // keep_comments の読み方
if let Some(before) = lex_as_compiler(src) {    // コメントを捨てた読み方
    let after = lex_as_compiler(&formatted).ok_or(OutputDoesNotLex)?;
    compare(&before, &after)?;
}
```

入力がコンパイラの読み方で lex できないときは比較対象がないので飛ばす。
そういうファイルを拒否しても、**壊れているファイルほど整形が要る**のに
format-on-save が効かなくなるだけで得がない。
...
全ステップを終えた時点で、リポジトリの `.er` 576ファイル中 81ファイルに
差分が出た。これを一括で当てて、CI の `build-check` ジョブが
`erg fmt --check .` で固定している（[使い方](erg-fmt.md)）。

差分の大半は1種類 — 100桁を超えた `.d.er` のシグネチャがカンマで折り返される。
`datetime.d.er` だけで40行増える。残りはインデント、カンマ周りの空白、
`+ - * /` の正規化、空行の連なり。
