...
- ~~`bool`: そもそも名前が定義されていない~~ → 追加済み（2026-09-05）。const のギャップではなく
  未実装の組み込みだった（`FUNC_BOOL` は PYTHON_MODE でしか使われていなかった）。実行時は
  他の変換関数と並べて `bool__`。**`Bool` は `int` の派生なので真偽値を先に取る**こと
  （`Bool(5)` は「5 である `Bool`」になる）。畳むのは真偽がここで決まる値だけで、型・
  サブルーチン・レコードは実行時に委ねる（const 位置では unfoldable として報告）
