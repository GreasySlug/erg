...
**進捗**: フェーズ 0〜5 は実装済み。フェーズ 6 は実測とメモ化のみ着手、本体は未着手（設計計画は策定済み: 段 0〜3 と分岐 D1〜D4）。
...
- ユーザークラスの const メソッドが畳めない件（上記）は段 0/1 と同じ「フレームの作り方」の
  領域にあるが、原因が同じかは未確認
...
| 7 | フェーズ 6（評価器再設計） | 着手（メモ化のみ）。本体は設計計画あり、D1〜D4 の判断待ち |
