...
`Pred`は述語式と呼ばれます。述語は「走らせて」検査されます。すなわち、ある値がその型に属するか
判定するたびに篩変数をその値に束縛し、述語をコンパイル時に評価します。したがって述語の中には
算術式もコンパイル時関数の呼び出しも書けます。

```python
{X: Int | X**2 - 5 * X + 6 == 0} # 2 と 3 はこの型に属し、5 は属さない
IsOdd(N: Int): Bool = N % 2 == 1
_: {N: Int | IsOdd(N)} = 7
```

コンパイラがしないのは述語を**解く**ことです。上の型が`{2, 3}`と同等であることは導けず、
渡された値が属するかどうかだけが分かります。
